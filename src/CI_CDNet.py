...
class CI_CDNet(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], downsample_mode='strideconv', upsample_mode='convtranspose', bias=True):
    ...
    def forward(self, x):
        ...
        x0_real = torch.real(x)
        x0_imag = torch.imag(x) 
...
def _calculate_correct_fan(tensor, mode):
    return torch.nn.init._calculate_correct_fan(tensor, mode)
