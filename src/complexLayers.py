...
class ComplexUpsample(Module):
    ...
    def forward(self, input):
        '''
        Uses the complex_upsample function to perform upsampling by separately interpolating
        the real and imaginary parts of the complex input and recombining them.
        '''
        return complex_upsample(input, size=self.size, scale_factor=self.scale_factor, 
                                mode=self.mode, align_corners=self.align_corners, 
                                recompute_scale_factor=self.recompute_scale_factor)


class ComplexUpsample2(Module):
    def __init__(self, size=None, scale_factor=None, mode='nearest',
                 align_corners=None, recompute_scale_factor=None):
        super(ComplexUpsample2, self).__init__()
        self.size = size
        self.scale_factor = scale_factor
        self.mode = mode
        self.align_corners = align_corners
    ...
    def forward(self, input):
        '''
        Uses the complex_upsample2 function to perform upsampling by separately interpolating
        the amplitude and phase parts of the complex input and recombining them.
        '''
        return complex_upsample2(input, size=self.size, scale_factor=self.scale_factor,
                                 mode=self.mode, align_corners=self.align_corners,
                                 recompute_scale_factor=self.recompute_scale_factor)
